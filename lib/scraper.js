...
URL_ADEUDOS_TENENCIA = 'http://data.finanzas.cdmx.gob.mx/consultas_pagos/consulta_adeudosten.php?consulta=Consulta+de+adeudos&wlplaca=';
URL_ADEUDO_TENENCIA = 'http://data.finanzas.cdmx.gob.mx/formato_lc/lc_new/tenencia/calculo/validaPlaca';
URL_INFRACCIONES = 'http://data.finanzas.cdmx.gob.mx/sma/detallePlaca.php?placa=';
...
module.exports.scrapeAdeudosTenencias = function(placa, callback) {
    ...
    x(URL_ADEUDOS_TENENCIA + placa, '#tabla500 tr', [{
        ...
        if (/.*adeudos*./.test(strTieneAdeudos.ejercicio)) {
            module.exports.scrapeInfoGeneral(placa, vehiculo, callback);
        } else {
...
module.exports.scrapeInfoGeneral = function(placa, vehiculo, callback) {
    var options = {
        url: URL_ADEUDO_TENENCIA,
        data: 'placa=' + placa + '&ejercicio=' + new Date().getFullYear()
    };
    curl.request(options, function(err, data) {
        data = JSON.parse(data);
        ...
        vehiculo.depreciacion_restante = truncaDecimales(vehiculo.valor_factura - vehiculo.depreciacion);
        callback(vehiculo);
        return;
    });
}
...
module.exports.scrapeInfracciones = function(placa, callback) {
    x(URL_INFRACCIONES + placa, '#tablaDatos', [{
        ...
        infracciones.forEach(function(infraccion) {
            ...
            infraccion.sancion = infraccion.sancion.trim()
            if (infraccion.situacion.trim() == 'Pagada' || infraccion.situacion.trim() == 'EXIMIDA') {
...
function calculaAdeudosTotales(vehiculo) {
